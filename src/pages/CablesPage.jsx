...
import { Cable, Plus, Search, Pencil, Trash2, X, Check, Network, ChevronDown, ChevronRight, Upload, Loader2, AlertTriangle, CheckCircle2 } from "lucide-react";
      ...
      >
        ...
        {!rows && !result && (
          <div className="space-y-4">
            ...
            <div className="border-2 border-dashed border-white/15 rounded-xl p-6 text-center">
              ...
              <p className="text-sm text-slate-400 mb-3">Select a CSV file</p>
              <input type="file" accept=".csv" onChange={e => setFile(e.target.files?.[0] || null)} className="block mx-auto text-xs text-slate-400 file:mr-3 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:bg-cyan-500/15 file:text-cyan-400 hover:file:bg-cyan-500/25" />
            </div>
        ...
        {rows && !result && (
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => setRows(null)} className="flex-1 py-2 rounded-lg border border-white/10 text-sm text-slate-400 hover:text-white transition-colors">Back</button>
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-6 space-y-6 animate-fade-in">
      ...
      <div className="glass rounded-xl overflow-hidden">
        ...
        {!loading && (
          <>
            <div className="hidden md:grid grid-cols-[80px_1fr_1fr_90px_100px_80px_80px_80px_72px] text-xs text-muted-foreground uppercase tracking-wide px-4 py-2.5 border-b border-border/50">
            ...
            <div className="divide-y divide-border/50">
              ...
              {filtered.map((c, i) => (
                <motion.div
                  ...
                  className="grid grid-cols-1 md:grid-cols-[80px_1fr_1fr_90px_100px_80px_80px_80px_72px] items-center gap-2 px-4 py-3 hover:bg-secondary/30 transition-colors"
                >
