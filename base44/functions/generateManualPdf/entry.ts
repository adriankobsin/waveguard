...
function addFooter(doc, state) {
  ...
  setRGB(doc, C.muted);
  doc.setFontSize(7.5);
  doc.text('Wave Guard — Complete Platform Manual', ML, PH - 8);
...
function buildCover(doc) {
  ...
  doc.setFontSize(36);
  setRGB(doc, C.white);
  doc.text('Wave Guard', PW / 2, 72, { align: 'center' });
...
function buildTOC(doc, state) {
  ...
  const entries = [
    ...
    { num: '1.1', title: 'What is Wave Guard?', page: 3 },
...
function buildSection1(doc, state) {
  ...
  y = subHeading(doc, y, '1.1  What is Wave Guard?', state);
  y = bodyText(doc, y, 'Wave Guard is an enterprise AV & network management platform designed for superyachts, large vessels, commercial facilities, and marine environments. It provides a unified interface for all technical systems on board, reducing the need for multiple separate management tools.', state);
  y += 2;
...
function buildSection3(doc, state) {
  ...
  y = infoBox(doc, y, '✓  After completing all 11 steps, Wave Guard is fully operational on your local network with auto-start on boot, HTTPS encryption, and local DNS access via https://guardian.local', C.green, state);
}
...
function buildSection5(doc, state) {
  ...
  const secItems = [
    ['Network Segmentation', [
      'Place the Wave Guard server on a dedicated Management VLAN (e.g. VLAN 10)',
      ...
      'Ensure SNMP polling traffic flows only from Wave Guard server IP',
    ]],
    ...
    ['Physical Security', [
      'Locate the Wave Guard server in a secure technical space (comms room, engine room)',
