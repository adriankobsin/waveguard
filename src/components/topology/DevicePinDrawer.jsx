...
const STATUS_CFG = {
  ...
  unknown: { label: "Unknown", color: "text-muted-foreground",   badge: "bg-slate-500/12 border-slate-500/30",     dot: "bg-slate-400",   bar: "bg-slate-500" },
};
  ...
  return (
    <div>
      <div className="flex justify-between text-[10px] mb-1">
        ...
        <span className="text-secondary-foreground font-medium">{value}%</span>
      </div>
...
export default function DevicePinDrawer({ pin, status, events, docs, onClose, onRemovePin }) {
  ...
  return (
    <motion.div
      ...
      className="w-80 flex-shrink-0 border-l border-border bg-card/98 backdrop-blur-xl flex flex-col shadow-2xl overflow-hidden"
    >
      {/* ── Header ── */}
      <div className="flex items-start justify-between px-5 py-4 border-b border-border">
        <div className="flex-1 min-w-0 pr-3">
          ...
          <p className="text-base font-bold text-foreground truncate">{device.name}</p>
          <p className="text-xs text-muted-foreground truncate">{device.model}</p>
        </div>
        <button onClick={onClose} className="w-7 h-7 rounded-lg hover:bg-muted flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors flex-shrink-0">
          <X size={14} />
        </button>
      </div>
        ...
        <div className="px-5 py-4 border-b border-border space-y-2">
          {[
            ...
            <div key={r.label} className="flex items-center justify-between">
              ...
              <span className={`text-xs font-medium ${r.colorClass || "text-foreground"} ${r.mono ? "font-mono" : ""}`}>{r.value}</span>
            </div>
          ))}
          {device.notes && (
            <p className="text-xs text-muted-foreground pt-1 border-t border-border leading-relaxed">{device.notes}</p>
          )}
        </div>
        ...
        {metrics && (
          <div className="px-5 py-4 border-b border-border">
            ...
            <div className="grid grid-cols-2 gap-2 mt-3">
              <div className="rounded-lg bg-muted border border-border px-2.5 py-2 text-center">
              ...
              <div className="rounded-lg bg-muted border border-border px-2.5 py-2 text-center">
                ...
                <p className="text-xs font-semibold text-foreground">{metrics.latency}</p>
              </div>
            </div>
          </div>
        )}
              ...
              {events.map(ev => (
                <div key={ev.id} className="flex items-start gap-2.5">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs text-secondary-foreground leading-snug">{ev.message}</p>
        ...
        <div className="px-5 py-4">
          ...
          {docs.length === 0 ? (
            ...
            <div className="space-y-2">
              {docs.map(doc => (
                <div key={doc.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-muted border border-border hover:bg-muted hover:border-border transition-colors group cursor-pointer">
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border flex-shrink-0 ${DOC_TYPE_COLORS[doc.type] || "bg-slate-500/15 text-muted-foreground border-slate-500/25"}`}>
                    {doc.type}
                  </span>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-foreground truncate group-hover:text-foreground transition-colors">{doc.name}</p>
                    <p className="text-[10px] text-muted-foreground">{doc.category} · {doc.size}</p>
                  </div>
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button className="w-6 h-6 flex items-center justify-center rounded-md hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" title="Open">
                      <ExternalLink size={11} />
                    </button>
                    <button className="w-6 h-6 flex items-center justify-center rounded-md hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" title="Download">
                      <Download size={11} />
                    </button>
                  </div>
                </div>
      ...
      <div className="px-5 py-4 border-t border-border flex gap-2">
        ...
        <button
          onClick={onRemovePin}
          className="w-10 h-9 flex items-center justify-center rounded-xl border border-border text-muted-foreground hover:text-red-400 hover:border-red-500/30 hover:bg-red-500/8 transition-colors"
          title="Remove pin"
        >
          <Trash2 size={13} />
        </button>
